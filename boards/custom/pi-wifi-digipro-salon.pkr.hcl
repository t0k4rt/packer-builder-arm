...
source "arm" "autogenerated_1" {
  file_checksum         = "a7eeb788247171277437e3dc1c946bfd"
  ...
  image_path                   = "pi-wifi-digipro-salon.img"
  image_size                   = "2G"
  image_type                   = "dos"
...
build {
  ...
  provisioner "shell" {
    inline = [
      // enable hifiberry digipro card
      "echo -n 'force_eeprom_read=0\n' | cat - /boot/config.txt | tee /boot/config.txt",
      "echo -n 'dtoverlay=hifiberry-digi-pro\n' | cat - /boot/config.txt | tee /boot/config.txt",
      ...
      // set host name
      "echo salon-digipro > /etc/hostname"]
  }
